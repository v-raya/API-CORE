...
/**
 * Taken from <a href=
 * "https://github.com/FasterXML/jackson-module-guice/tree/master/src/test/java/com/fasterxml/jackson/module/guice">Jackson
 * Guice examples</a>. Verifies annotation-driven translation of system codes.
 * 
...
public class ObjectMapperModuleTest {
  ...
  public void testModulesRegisteredThroughNormalInstantiation() throws Exception {
    final Injector injector =
        Guice.createInjector(new ObjectMapperModule().registerModule(new IntegerAsBase16Module()));
    final ObjectMapper mapper = injector.getInstance(ObjectMapper.class);
  ...
  public void testModulesRegisteredThroughInjection() throws Exception {
    final Injector injector =
        Guice.createInjector(new ObjectMapperModule().registerModule(IntegerAsBase16Module.class));
    final ObjectMapper mapper = injector.getInstance(ObjectMapper.class);
    Assert.assertEquals(mapper.writeValueAsString(new Integer(10)), "\"A\"");
  }
  ...
  public void testModulesRegisteredThroughInjectionWithAnnotation() throws Exception {
    final Injector injector = Guice.createInjector(new ObjectMapperModule()
        .registerModule(IntegerAsBase16Module.class, SystemCodeSerializer.class), new Module() {
          @Override
          public void configure(Binder binder) {
            binder.bind(IntegerAsBase16Module.class).annotatedWith(SystemCodeSerializer.class)
  ...
  public void testModulesRegisteredThroughInjectionWithNameAnnotation() throws Exception {
    final Injector injector = Guice.createInjector(
        ...
        new Module() {
          @Override
          public void configure(Binder binder) {
            binder.bind(IntegerAsBase16Module.class).annotatedWith(Names.named("billy"))
                .to(IntegerAsBase16Module.class);
          }
        });
  ...
  private static class SomeBean {
    ...
    @JacksonInject
    private void injectEight(@SystemCodeSerializer long eight) {
      this.eight = eight;
    }
    ...
    public IntegerAsBase16Module() {
      ...
      addSerializer(Integer.class, new JsonSerializer<Integer>() {
        ...
        public void serialize(Integer integer, JsonGenerator jsonGenerator,
          ...
          jsonGenerator
              .writeString(new BigInteger(String.valueOf(integer)).toString(16).toUpperCase());
        }
      });
    }
  }
