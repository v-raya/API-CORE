   ...
   stage('SonarQube analysis'){
	...
	stage ('Push to artifactory'){
	  ...
	  if (params.APP_VERSION != "SNAPSHOT") {
        ...
        buildInfo = rtGradle.run buildFile: 'build.gradle', tasks: 'clean publish -Dversion=${APP_VERSION}'
	  } else {
	      echo "!!!! PUSH SNAPSHOT VERSION"
	      rtGradle.deployer repo:'libs-snapshot', server: serverArti
	      rtGradle.deployer.deployArtifacts = true
        buildInfo = rtGradle.run buildFile: 'build.gradle', tasks: 'clear publish'
	  }
