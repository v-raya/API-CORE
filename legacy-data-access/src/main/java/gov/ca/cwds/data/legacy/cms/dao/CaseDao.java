...
public class CaseDao extends CrudsDaoImpl<Case> {
  ...
  /**
   * Returns cases which have relation to {@link StaffPerson} via {@link CaseAssignment},
   * {@link CaseLoad} and {@link CaseLoadWeighting} entities. Also the {@link CaseAssignment} is
   ...
   */
  public Collection<CaseByStaff> nativeFindByStaffIdAndActiveDate(final String staffId,
    ...
    final List<CaseByStaff> cases = currentSession()
  ...
  public List<Case> findActiveByClient(String clientId) {
    ...
    List<Case> cases =
        currentSession().createNamedQuery(Case.NQ_FIND_ACTIVE_BY_CLIENT_ID, Case.class)
            .setParameter(Case.NQ_PARAM_CLIENT_ID, clientId).list();
  ...
  public List<Case> findClosedByClient(String clientId) {
    List<Case> cases =
        currentSession().createNamedQuery(Case.NQ_FIND_CLOSED_BY_CLIENT_ID, Case.class)
            .setParameter(Case.NQ_PARAM_CLIENT_ID, clientId).list();
