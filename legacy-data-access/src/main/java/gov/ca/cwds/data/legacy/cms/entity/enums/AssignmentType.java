 ...
 * or indirect assignment. Similarly, Secondary assigned workers also have the same capabilities
 * to retrieve, read, and update any case or referral to which they have direct or indirect assignment.
 ...
 * from doing something that the Primary worker can do (i.e. requesting the closure of
 * an Adoptions Case). Read Only assigned workers have the capability to access and read any case
 * or referral to which they have direct or indirect assignment, but they do not have update capability.
 *
...
public enum AssignmentType implements EntityEnum<String> {
  ...
  public static AssignmentType from(String code) {
    return Arrays.asList(AssignmentType.values())
        .stream()
        .findFirst()
