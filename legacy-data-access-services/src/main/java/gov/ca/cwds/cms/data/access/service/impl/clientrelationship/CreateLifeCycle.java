...
class CreateLifeCycle extends CreateUpdateBaseLifeCycle {
  ...
  @Inject
  CreateLifeCycle(ClientRelationshipDao clientRelationshipDao,
    ...
    super(clientRelationshipDao, businessValidationService, clientDao,
        tribalMembershipVerificationDao, paternityDetailDao, searchClientRelationshipService);
  ...
  private void createChildDuplicateTribalIfNeeded(DataAccessBundle bundle)
    ...
    final List<TribalMembershipVerification> parentRelations =
        tribalMembershipVerificationDao.findByClientId(parent.getIdentifier());
    if (CollectionUtils.isEmpty(parentRelations)) {
      return;
    }

    final List<TribalMembershipVerification> tribalsWithSubTribals = tribalMembershipVerificationDao
        .findTribalsThatHaveSubTribalsByClientId(child.getIdentifier(), parent.getIdentifier());

    if (CollectionUtils.isNotEmpty(tribalsWithSubTribals)) {
      return;
    }

    final TribalMembershipVerificationAwareDto tribalMembershipVerificationAwareDto =
        new TribalMembershipVerificationAwareDto(parent.getIdentifier(), child.getIdentifier());
    final TribalMembershipVerification tribalEntity = parentRelations.get(0);
